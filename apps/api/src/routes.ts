...
export function registerCoreRoutes(app:FastifyInstance){
  ...
  app.get('/elections/:id', {preHandler:[app.authenticate]}, async(req:any)=>prisma.election.findUnique({where:{id:req.params.id},include:{candidates:{include:{user:{select:{profile:{select:{fullName:true,avatarUrl:true}}}}}}}}));
