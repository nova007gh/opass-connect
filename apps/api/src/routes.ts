...
export function registerCoreRoutes(app:FastifyInstance){
  ...
  app.get('/elections/:id', {preHandler:[app.authenticate]}, async(req:any)=>prisma.election.findUnique({where:{id:req.params.id},include:{candidates:true}}));
  app.get('/elections/:id/results',{preHandler:[app.authenticate]},async(req:any,reply)=>{const election=await prisma.election.findUnique({where:{id:req.params.id}});if(!election)return reply.code(404).send({error:'Election not found'});if(!['CLOSED','CERTIFIED'].includes(election.status)&&!['ADMIN','SUPER_ADMIN'].includes(req.user.role))return reply.code(403).send({error:'Results are not public yet'});return prisma.vote.groupBy({by:['candidateId','position'],where:{electionId:election.id},_count:{_all:true}})});
  app.post('/elections/:id/vote',{preHandler:[app.authenticate]},async(req:any,reply)=>{const b=z.object({candidateId:z.string(),position:z.string()}).parse(req.body);const e=await prisma.election.findUnique({where:{id:req.params.id}});if(!e||e.status!=='OPEN'||new Date()<e.opensAt||new Date()>e.closesAt)return reply.code(409).send({error:'Election is not open'});const candidate=await prisma.candidate.findFirst({where:{id:b.candidateId,electionId:e.id,position:b.position}});if(!candidate)return reply.code(400).send({error:'Invalid candidate'});try{return await prisma.vote.create({data:{electionId:e.id,candidateId:b.candidateId,voterId:req.user.sub,position:b.position}})}catch{return reply.code(409).send({error:'You have already voted for this position'});}});
