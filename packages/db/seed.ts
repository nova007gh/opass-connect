import { PrismaClient, UserRole, VerificationStatus, ElectionStatus, ProjectStatus } from '@prisma/client';
...
async function main() {
  ...
  // ===== Year groups =====
  const yg2006 = await prisma.yearGroup.upsert({
    where: { year: 2006 },
    update: {},
    create: { year: 2006, name: 'Class of 2006', description: 'Ofori Panin Senior High School Class of 2006' },
  });
  const yg2010 = await prisma.yearGroup.upsert({
    where: { year: 2010 },
    update: {},
    create: { year: 2010, name: 'Class of 2010', description: 'Ofori Panin Senior High School Class of 2010' },
  });
  const yg2015 = await prisma.yearGroup.upsert({
    where: { year: 2015 },
    update: {},
    create: { year: 2015, name: 'Class of 2015', description: 'Ofori Panin Senior High School Class of 2015' },
  });
  console.log('Year groups:', yg2006.name, yg2010.name, yg2015.name);
  ...
  for (const a of alumniData) {
      ...
      create: {
        ...
        profile: {
          create: {
            ...
            country: a.country,
            city: a.city,
            bio: `OPASS alumni, Class of ${a.year}. ${a.profession} based in ${a.city}, ${a.country}.`,
          },
        },
      },
      include: { profile: true },
    ...
    await prisma.yearGroupMembership.upsert({
      where: { userId_yearGroupId: { userId: user.id, yearGroupId: yg.id } },
      update: {},
      create: { userId: user.id, yearGroupId: yg.id },
    });
  }
  console.log(`Created ${members.length} alumni members (password: opassmember2026)`);

  // ===== Events =====
  await prisma.event.create({
    data: {
      title: 'OPASS Global Reunion 2026',
      description: 'Annual reunion for all OPASS alumni worldwide. Networking, dinner and entertainment.',
      startsAt: new Date('2026-12-15T10:00:00Z'),
      endsAt: new Date('2026-12-15T18:00:00Z'),
      venue: 'OPASS Campus, Kukurantumi',
    },
  }).catch(() => null);

  await prisma.event.create({
    data: {
      title: '2006 Year Group 20th Anniversary',
      description: 'Celebrating 20 years since the Class of 2006 graduated from OPASS.',
      startsAt: new Date('2026-07-04T09:00:00Z'),
  ...
  await prisma.event.create({
    data: {
      title: 'OPASS Career Mentorship Webinar',
      description: 'Virtual mentorship session for current OPASS students. Alumni professionals share career advice.',
      startsAt: new Date('2026-09-20T15:00:00Z'),
      endsAt: new Date('2026-09-20T17:00:00Z'),
      streamUrl: 'https://youtube.com/live/opass-mentorship',
    },
  }).catch(() => null);
  ...
  await prisma.project.create({
    data: {
      ...
      description: 'Help us renovate and modernize the OPASS school library for current students. Target includes new books, computers and furniture.',
      targetAmount: 50000,
      raisedAmount: 12500,
  ...
  await prisma.project.create({
    data: {
      ...
      description: 'Purchase of modern lab equipment for the science departments. Every contribution counts!',
      targetAmount: 30000,
      raisedAmount: 28500,
      status: ProjectStatus.ACTIVE,
      yearGroupId: yg2006.id,
    },
  }).catch(() => null);

  await prisma.project.create({
    data: {
      title: 'Sports Field Upgrade',
      description: 'Upgrading the football field and athletics track to benefit all students.',
      targetAmount: 75000,
      raisedAmount: 75000,
  ...
  for (let i = 0; i < bizOwners.length; i++) {
    await prisma.business.create({
      data: { ownerId: bizOwners[i].id, ...businesses[i], verified: true },
    }).catch(() => null);
  }
  console.log('Businesses created');

  // ===== Chat rooms =====
  const assemblyHall = await prisma.chatRoom.create({
    data: { name: 'OPASS Assembly Hall', isAssemblyHall: true },
  }).catch(() => null);
  const class2006Room = await prisma.chatRoom.create({
    data: { name: 'Class of 2006 Discussion', yearGroupId: yg2006.id },
  }).catch(() => null);
  const generalRoom = await prisma.chatRoom.create({
    data: { name: 'General Alumni Lounge' },
  }).catch(() => null);
  ...
  const election = await prisma.election.create({
    data: {
      title: '2006 Year Group President Election',
      description: 'Election for the Class of 2006 Year Group President. One member, one vote.',
      yearGroupId: yg2006.id,
  ...
  console.log('Election created with candidates');

  // ===== Meeting =====
  await prisma.meeting.create({
    data: {
      title: 'OPASS Global Town Hall',
      description: 'Quarterly town hall meeting for all alumni. Updates from the association, Q&A and networking.',
      mode: 'WEBINAR',
      status: 'SCHEDULED',
      startsAt: new Date('2026-09-01T18:00:00Z'),
      capacity: 5000,
      roomKey: `opass-${crypto.randomUUID()}`,
      hostUserId: admin.id,
    },
  }).catch(() => null);
  ...
  await prisma.meeting.create({
    data: {
      title: '2006 Class Reunion Planning Call',
      description: 'Planning meeting for the 20th anniversary reunion.',
      mode: 'INTERACTIVE',
      status: 'SCHEDULED',
      startsAt: new Date('2026-06-15T19:00:00Z'),
      capacity: 100,
      roomKey: `opass-${crypto.randomUUID()}`,
      hostUserId: members[0]?.id || admin.id,
      yearGroupId: yg2006.id,
    },
  }).catch(() => null);
  ...
  console.log('  Email:    admin@opassconnect.edu.gh');
  console.log('  Password: opassadmin2026');
  console.log('Member login:');
  console.log('  Email:    kwame.mensah@gmail.com');
  console.log('  Password: opassmember2026');
  console.log('========================================');
}
