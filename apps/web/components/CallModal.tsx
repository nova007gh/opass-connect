...
export default function CallModal({ callType, peerName, peerAvatarUrl, connect, onClose, isGroupCall }: CallModalProps) {
  ...
  // Helper to update a remote participant's info
  ...
  const detachRemoteTrack = useCallback((track: RemoteTrack, participant: RemoteParticipant) => {
    track.detach();
    updateRemoteParticipant(participant);
  }, [updateRemoteParticipant]);
  ...
  useEffect(() => {
    ...
    room.on(RoomEvent.TrackUnsubscribed, (track, pub, participant) => {
      detachRemoteTrack(track, participant as RemoteParticipant);
    });
  ...
  // Auto-hide controls after 5 seconds of inactivity (video mode only)
  ...
  const qualityIcon = (q: ConnectionQuality) => {
    if (q === ConnectionQuality.Excellent) return '●●●';
    if (q === ConnectionQuality.Good) return '●●○';
    if (q === ConnectionQuality.Poor) return '●○○';
    return '○○○';
  };

  const qualityColor = (q: ConnectionQuality) => {
    if (q === ConnectionQuality.Excellent) return '#22C55E';
    if (q === ConnectionQuality.Good) return '#EAB308';
    if (q === ConnectionQuality.Poor) return '#EF4444';
    return '#6B7280';
  };

  const remoteList = Array.from(remoteParticipants.values());
  const hasRemoteVideo = remoteList.some(p => p.hasVideo);
  ...
  if (callType === 'audio') {
    return (
      <div
        style={{
          position: 'fixed', inset: 0, background: 'linear-gradient(180deg, #0B2D6B 0%, #0F172A 100%)',
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ color: 'white', fontSize: 14, fontWeight: 600 }}>
            {isGroupCall ? `${peerName} · ${participantCount} in call` : peerName}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: qualityColor(localQuality), fontSize: 10, fontWeight: 700 }}>{qualityIcon(localQuality)}</span>
            <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, fontVariantNumeric: 'tabular-nums' }}>
              {status === 'connecting' ? 'Connecting…' : status === 'reconnecting' ? 'Reconnecting…' : status === 'error' ? 'Error' : fmtDuration(duration)}
            </span>
          </div>
        </div>
          ...
          {status === 'error' ? (
            <div style={{ textAlign: 'center', padding: 20 }}>
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(239,68,68,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                <svg fill="none" stroke="#EF4444" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 36, height: 36 }}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" /></svg>
              </div>
              <div style={{ color: '#FCA5A5', fontSize: 15, fontWeight: 600, marginBottom: 16 }}>{error}</div>
              <button onClick={retry} style={{ background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 999, padding: '10px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Retry</button>
            </div>
          ) : (
            <>
              {/* Pulsing avatar */}
              <div style={{ position: 'relative', width: 140, height: 140 }}>
                ...
                <div style={{
                  ...
                  background: 'linear-gradient(135deg, #0051FF 0%, #0B2D6B 100%)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 52, fontWeight: 800, color: 'white', position: 'relative',
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14, marginTop: 6 }}>
                  {status === 'connecting' ? 'Connecting…' : status === 'reconnecting' ? 'Reconnecting…' : isGroupCall && remoteList.length === 0 ? 'Waiting for others to join…' : fmtDuration(duration)}
                </div>
                {isGroupCall && remoteList.length > 0 && (
                  <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12, marginTop: 8 }}>
                    {remoteList.map(p => p.name).join(', ')} {remoteList.length === 1 ? 'is' : 'are'} in the call
                  </div>
                )}
              </div>
            </>
        ...
        {remoteList.map(p => (
          <audio key={p.identity} ref={el => { if (el) remoteAudioRefs.current.set(p.identity, el); }} autoPlay />
        ))}

        {/* Bottom controls */}
        <div style={{
          padding: '24px 20px', paddingBottom: 'calc(40px + env(safe-area-inset-bottom, 0px))',
          ...
          <button onClick={toggleMic} style={{
            ...
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            WebkitBackdropFilter: 'blur(8px)', transition: 'transform 0.15s',
          }} onTouchStart={(e) => e.currentTarget.style.transform = 'scale(0.92)'}
            onTouchEnd={(e) => e.currentTarget.style.transform = 'scale(1)'}
            title={micOn ? 'Mute' : 'Unmute'}>
            {micOn ? (
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 28, height: 28 }}><path strokeLinecap="round" strokeLinejoin="round" d="M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z" /></svg>
            ) : (
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 28, height:28 }}><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.25l13.5 13.5M15 10.5V4.5a3 3 0 00-5.94-.6M12 18.75a6 6 0 006-6v-1.5m-9 4.243A5.978 5.978 0 016 12.75v-1.5m6 7.5v3.75m-3.75 0h7.5" /></svg>
            )}
          </button>
          <button onClick={hangUp} style={{
            width: 72, height: 72, borderRadius: '50%', border: 0, cursor: 'pointer',
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden', background: '#000' }}>
        {/* Group call — grid of participants */}
        {isGroupCall && remoteList.length > 0 ? (
          <div style={{
            width: '100%', height: '100%', display: 'grid',
            gridTemplateColumns: remoteList.length <= 1 ? '1fr' : remoteList.length <= 4 ? '1fr 1fr' : '1fr 1fr 1fr',
            gridTemplateRows: remoteList.length <= 2 ? '1fr' : '1fr 1fr',
            gap: 2, padding: 2,
          }}>
            {/* Remote participants */}
            {remoteList.map(p => (
              <div key={p.identity} style={{
                position: 'relative', background: '#1a1a2e', borderRadius: 4, overflow: 'hidden',
                border: activeSpeakerId === p.identity ? '2px solid #22C55E' : '2px solid transparent',
              }}>
                {p.hasVideo ? (
                  <video
                ...
                <div style={{
                  ...
                  maxWidth: 'calc(100% - 8px)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                }}>
                  {p.name}
                </div>
                {/* Mic indicator */}
                {!p.hasAudio && (
                  <div style={{
                    ...
                    <svg fill="none" stroke="white" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 12, height: 12 }}><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.25l13.5 13.5M15 10.5V4.5a3 3 0 00-5.94-.6" /></svg>
                  </div>
                )}
              </div>
            ))}
          ...
          /* 1-on-1 video — full screen remote */
          <>
            <video
              ref={el => { if (el) remoteVideoRefs.current.set(remoteList[0].identity, el); }}
              autoPlay playsInline
              style={{ width: '100%', height: '100%', objectFit: 'cover' }}
            />
          ...
          /* No remote video — show avatar + connecting state */
            ...
            {status === 'error' ? (
              ...
              <>
                ...
                <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 14 }}>
                  {status === 'connecting' ? 'Connecting…' : status === 'reconnecting' ? 'Reconnecting…' : isGroupCall ? 'Waiting for others to join…' : 'Ringing…'}
                </div>
                {isGroupCall && status === 'connected' && (
                  <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginTop: 4, textAlign: 'center', maxWidth: 280 }}>
                    Others can join from the green banner in the chat
                  </div>
                )}
              </>
            )}
        ...
        {/* Hidden audio elements for any remaining participants */}
        {remoteList.filter(p => !(isGroupCall || (!isGroupCall && p.identity === remoteList[0]?.identity && p.hasVideo))).map(p => (
          <audio key={p.identity} ref={el => { if (el) remoteAudioRefs.current.set(p.identity, el); }} autoPlay />
        ))}

        {/* Local video PiP — draggable-style corner placement */}
        {camOn && (
          <div style={{
            position: 'absolute', top: 'calc(12px + env(safe-area-inset-top, 0px))', right: 12,
            width: '28vw', maxWidth: 140, minWidth: 90,
            aspectRatio: '9 / 16',
        ...
        <div style={{
          ...
          {status === 'connected' && (
            <span style={{ color: 'rgba(255,255,255,0.6)', fontVariantNumeric: 'tabular-nums' }}>· {fmtDuration(duration)}</span>
          )}
          {status === 'reconnecting' && <span style={{ color: '#EAB308' }}>· Reconnecting</span>}
        </div>
      ...
      {/* Bottom controls */}
      <div style={{
        padding: '20px 16px', paddingBottom: 'calc(28px + env(safe-area-inset-bottom, 0px))',
        ...
        <button onClick={toggleMic} style={controlBtnStyle(micOn)} title={micOn ? 'Mute' : 'Unmute'}>
          {micOn ? (
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 26, height: 26 }}><path strokeLinecap="round" strokeLinejoin="round" d="M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z" /></svg>
          ) : (
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2} style={{ width: 26, height: 26 }}><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.25l13.5 13.5M15 10.5V4.5a3 3 0 00-5.94-.6M12 18.75a6 6 0 006-6v-1.5m-9 4.243A5.978 5.978 0 016 12.75v-1.5m6 7.5v3.75m-3.75 0h7.5" /></svg>
          )}
        </button>

        {callType === 'video' && (
          <button onClick={toggleCam} style={controlBtnStyle(camOn)} title={camOn ? 'Turn off camera' : 'Turn on camera'}>
            {camOn ? (
        ...
        {callType === 'video' && (
          <button onClick={flipCamera} style={controlBtnStyle(true)} title="Flip camera">
        ...
        <button onClick={hangUp} style={{
          width: 68, height: 68, borderRadius: '50%', border: 0, cursor: 'pointer',
          background: '#EF4444', color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 4px 20px rgba(239,68,68,0.4)', transition: 'transform 0.15s',
          flexShrink: 0,
...
function controlBtnStyle(active: boolean): React.CSSProperties {
  return {
    ...
    background: active ? 'rgba(255,255,255,0.15)' : 'white',
