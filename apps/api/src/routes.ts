...
export function registerCoreRoutes(app:FastifyInstance){
  ...
  app.get('/year-groups',{preHandler:[app.authenticate]},async(req:any)=>{
    ...
    const [groups,withInvites]=await Promise.all([
      prisma.yearGroup.findMany({where,orderBy:{year:'desc'},take:100,include:{_count:{select:{memberships:{where:{banned:false}}}}}}),
      canManageAny?prisma.yearGroupInvite.groupBy({by:['yearGroupId'],where:{status:'PENDING'},_count:{_all:true}}):Promise.resolve([]),
    ]);
  ...
  app.get('/year-groups/:id/posts',{preHandler:[app.authenticate]},async(req:any,reply)=>{
    ...
    const posts=await prisma.yearGroupPost.findMany({where:{yearGroupId:yg.id},orderBy:{createdAt:'desc'},take:q.limit,...(q.cursor?{skip:1,cursor:{id:q.cursor}}:{}),include:{user:{select:{id:true,role:true,profile:{select:{fullName:true,avatarUrl:true,house:true}}}},_count:{select:{likes:true,comments:true}},likes:{where:{userId:req.user.sub},select:{id:true}}}});
  ...
  app.get('/year-groups/:id/posts/:postId/comments',{preHandler:[app.authenticate]},async(req:any,reply)=>{
    ...
    return prisma.yearGroupPostComment.findMany({where:{postId:req.params.postId},orderBy:{createdAt:'asc'},include:{user:{select:{id:true,role:true,profile:{select:{fullName:true,avatarUrl:true,house:true}}}}}});
  });
  ...
  app.get('/alumni',{preHandler:[app.authenticate]},async(req:any)=>{const q=z.object({year:z.coerce.number().optional(),house:z.string().optional(),search:z.string().optional(),limit:z.coerce.number().int().min(1).max(100).optional()}).parse(req.query);const profiles=await prisma.alumniProfile.findMany({where:{searchable:true,graduationYear:q.year,house:q.house,fullName:q.search?{contains:q.search,mode:'insensitive'}:undefined},take:q.limit??100,orderBy:{fullName:'asc'},select:{fullName:true,nickname:true,graduationYear:true,house:true,country:true,city:true,profession:true,bio:true,avatarUrl:true,userId:true,user:{select:{role:true}}}});return profiles.map(({user,...p})=>({...p,role:user?.role||'MEMBER'}))});

  // ===== Direct messages (1-on-1 chat) =====
  app.get('/dm/conversations',{preHandler:[app.authenticate]},async(req:any)=>{
    // Get all unique conversation partners — limit to recent 200 messages for performance
    const [sent,received]=await Promise.all([
      prisma.directMessage.findMany({where:{senderId:req.user.sub},select:{recipientId:true,createdAt:true,body:true,callType:true},orderBy:{createdAt:'desc'},take:200}),
      prisma.directMessage.findMany({where:{recipientId:req.user.sub},select:{senderId:true,createdAt:true,body:true,callType:true},orderBy:{createdAt:'desc'},take:200}),
    ]);
    ...
    const userIds=[...partners.keys()].slice(0,50);
    if(userIds.length===0)return[];
    const users=await prisma.user.findMany({where:{id:{in:userIds}},select:{id:true,email:true,role:true,profile:{select:{fullName:true,avatarUrl:true,graduationYear:true,profession:true,house:true}}}});
  ...
  app.get('/dm/:userId',{preHandler:[app.authenticate]},async(req:any,reply)=>{
    ...
    const otherUser=await prisma.user.findUnique({where:{id:req.params.userId},select:{id:true,email:true,role:true,profile:{select:{fullName:true,avatarUrl:true,graduationYear:true,profession:true,house:true,country:true,city:true,bio:true}}}});
  ...
  app.get('/projects',async()=>prisma.project.findMany({where:{status:{in:['ACTIVE','FUNDED','IN_PROGRESS','COMPLETED']}},orderBy:{createdAt:'desc'},take:50}));
  ...
  app.get('/events',async()=>prisma.event.findMany({orderBy:{startsAt:'asc'},take:50}));
  ...
  app.get('/chat/rooms',{preHandler:[app.authenticate]},async()=>prisma.chatRoom.findMany({orderBy:{createdAt:'asc'},take:100,include:{_count:{select:{messages:true}},yearGroup:{select:{year:true,name:true}}}}));
  ...
  app.get('/chat/rooms/:id/messages',{preHandler:[app.authenticate]},async(req:any)=>{const q=z.object({cursor:z.string().optional(),limit:z.coerce.number().int().min(1).max(100).default(50)}).parse(req.query);return prisma.message.findMany({where:{roomId:req.params.id},orderBy:{createdAt:'desc'},take:q.limit,...(q.cursor?{skip:1,cursor:{id:q.cursor}}:{}) ,include:{user:{select:{id:true,role:true,profile:{select:{fullName:true,avatarUrl:true,house:true}}}},replyTo:{select:{id:true,body:true,userId:true,audioUrl:true,imageUrl:true,videoUrl:true,fileUrl:true,fileName:true,locationLat:true,locationLng:true,user:{select:{profile:{select:{fullName:true}}}}}}}})});
  ...
  app.get('/elections',{preHandler:[app.authenticate]},async()=>prisma.election.findMany({orderBy:{opensAt:'desc'},take:50,include:{_count:{select:{candidates:true,votes:true}},yearGroup:{select:{year:true,name:true}}}}));
  ...
  app.post('/admin/quotes/:id/reject',{preHandler:[app.authenticate,requireRoles('ADMIN','SUPER_ADMIN')]},async(req:any)=>{const quote=await prisma.quote.update({where:{id:req.params.id},data:{status:'REJECTED'},include:{intake:true}});if(quote.intake?.clientEmail){const{sendEmail}=await import('./email.js');sendEmail(quote.intake.clientEmail,`Quote ${quote.quoteNumber} Update`,`Your quote status has been updated`,`<p>Your quote <strong>${quote.quoteNumber}</strong> has been rejected. Please contact OPASS for more information.</p>`).catch(()=>{});}return quote;});
  ...
  app.post('/admin/users/:userId/reset-password',{preHandler:[app.authenticate,requireRoles('ADMIN','SUPER_ADMIN')]},async(req:any,reply)=>{
    const body=z.object({newPassword:z.string().min(10)}).parse(req.body);
