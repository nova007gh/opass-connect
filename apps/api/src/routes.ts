...
export function registerCoreRoutes(app:FastifyInstance){
  ...
  app.get('/alumni',{preHandler:[app.authenticate]},async(req:any)=>{const q=z.object({year:z.coerce.number().optional(),house:z.string().optional(),search:z.string().optional(),limit:z.coerce.number().int().min(1).max(100).optional()}).parse(req.query);const profiles=await prisma.alumniProfile.findMany({where:{searchable:true,graduationYear:q.year,house:q.house,fullName:q.search?{contains:q.search,mode:'insensitive'}:undefined},take:q.limit??100,orderBy:{fullName:'asc'},select:{fullName:true,nickname:true,graduationYear:true,house:true,country:true,city:true,profession:true,bio:true,avatarUrl:true,userId:true}});const userIds=profiles.map(p=>p.userId);const users=await prisma.user.findMany({where:{id:{in:userIds}},select:{id:true,role:true}});const roleMap=new Map(users.map(u=>[u.id,u.role]));return profiles.map(p=>({...p,role:roleMap.get(p.userId)||'MEMBER'}))});
