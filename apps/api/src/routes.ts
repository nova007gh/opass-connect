...
export function registerCoreRoutes(app:FastifyInstance){
  ...
  app.get('/year-groups/:id/invites',{preHandler:[app.authenticate]},async(req:any,reply)=>{const yg=await prisma.yearGroup.findUnique({where:{id:req.params.id}});if(!yg)return reply.code(404).send({error:'Year group not found'});if(!canManageGroup(req.user,yg))return reply.code(403).send({error:'Only the group creator or an admin can view invites'});return prisma.yearGroupInvite.findMany({where:{yearGroupId:req.params.id},orderBy:{createdAt:'desc'},include:{invitedUser:{select:{email:true,profile:{select:{fullName:true,avatarUrl:true,graduationYear:true}}}},invitedBy:{select:{email:true,profile:{select:{fullName:true}}}}}});});
  app.post('/year-groups/:id/invite',{preHandler:[app.authenticate]},async(req:any,reply)=>{const yg=await prisma.yearGroup.findUnique({where:{id:req.params.id}});if(!yg)return reply.code(404).send({error:'Year group not found'});if(!canManageGroup(req.user,yg))return reply.code(403).send({error:'Only the group creator or an admin can invite members'});const b=z.object({userId:z.string()}).parse(req.body);const alreadyMember=await prisma.yearGroupMembership.findUnique({where:{userId_yearGroupId:{userId:b.userId,yearGroupId:yg.id}}});if(alreadyMember)return reply.code(409).send({error:'User is already a member of this group'});const isAdmin=['ADMIN','SUPER_ADMIN'].includes(req.user.role);const invite=await prisma.yearGroupInvite.upsert({where:{yearGroupId_invitedUserId:{yearGroupId:yg.id,invitedUserId:b.userId}},update:{status:isAdmin?'APPROVED':'PENDING',invitedByUserId:req.user.sub},create:{yearGroupId:yg.id,invitedUserId:b.userId,invitedByUserId:req.user.sub,status:isAdmin?'APPROVED':'PENDING'}});if(isAdmin){await prisma.yearGroupMembership.upsert({where:{userId_yearGroupId:{userId:b.userId,yearGroupId:yg.id}},update:{},create:{userId:b.userId,yearGroupId:yg.id}});notifyUser(b.userId,'SYSTEM',`You've been added to ${yg.name}`,`An admin added you to ${yg.name}.`,'/dashboard/groups',true).catch(()=>{});}else{notifyAdmins('SYSTEM','Year group invite needs approval',`A member was invited to join ${yg.name} and needs your approval.`,'/dashboard/admin').catch(()=>{});notifyUser(b.userId,'SYSTEM',`You've been invited to ${yg.name}`,`You'll be added once an admin approves the invite.`,'/dashboard/groups').catch(()=>{});}return invite;});
  ...
  app.post('/year-group-invites/:id/approve',{preHandler:[app.authenticate]},async(req:any,reply)=>{const invite=await prisma.yearGroupInvite.findUnique({where:{id:req.params.id},include:{yearGroup:true}});if(!invite)return reply.code(404).send({error:'Invite not found'});if(!canManageGroup(req.user,invite.yearGroup))return reply.code(403).send({error:'Only the group creator or an admin can approve invites'});await prisma.$transaction([prisma.yearGroupInvite.update({where:{id:invite.id},data:{status:'APPROVED'}}),prisma.yearGroupMembership.upsert({where:{userId_yearGroupId:{userId:invite.invitedUserId,yearGroupId:invite.yearGroupId}},update:{},create:{userId:invite.invitedUserId,yearGroupId:invite.yearGroupId}})]);notifyUser(invite.invitedUserId,'SYSTEM',`Welcome to ${invite.yearGroup.name}!`,`Your request to join ${invite.yearGroup.name} was approved.`,'/dashboard/groups',true).catch(()=>{});return{ok:true};});
  app.post('/year-group-invites/:id/reject',{preHandler:[app.authenticate]},async(req:any,reply)=>{const invite=await prisma.yearGroupInvite.findUnique({where:{id:req.params.id},include:{yearGroup:true}});if(!invite)return reply.code(404).send({error:'Invite not found'});if(!canManageGroup(req.user,invite.yearGroup))return reply.code(403).send({error:'Only the group creator or an admin can reject invites'});await prisma.yearGroupInvite.update({where:{id:invite.id},data:{status:'REJECTED'}});notifyUser(invite.invitedUserId,'SYSTEM',`Update on ${invite.yearGroup.name}`,`Your request to join ${invite.yearGroup.name} was not approved.`,'/dashboard/groups').catch(()=>{});return{ok:true};});
  ...
  app.get('/admin/year-group-invites',{preHandler:[app.authenticate,requireRoles('ADMIN','SUPER_ADMIN')]},async(req:any)=>{const q=z.object({status:z.string().optional()}).parse(req.query);return prisma.yearGroupInvite.findMany({where:q.status?{status:q.status as any}:{status:'PENDING'},orderBy:{createdAt:'desc'},include:{yearGroup:{select:{year:true,name:true}},invitedUser:{select:{email:true,profile:{select:{fullName:true,avatarUrl:true,graduationYear:true}}}},invitedBy:{select:{email:true,profile:{select:{fullName:true}}}}}});});
